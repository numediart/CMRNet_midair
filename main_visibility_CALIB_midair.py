...
from DatasetVisibilityMidair import DatasetVisibilityMidairSingle
...
def config():
    ...
    dataset = 'midair'
    data_folder = '/home/ambroise/dataset/MidAir/PLE_training/spring'
    use_reflectance = False
    test_sequence = 'trajectory_5001'
...
def main(_config, _run, seed):
    ...
    if _config['test_sequence'] is None:
        ...
        _config['test_sequence'] = f"{_config['test_sequence']}"
        print("Test Sequence: ", _config['test_sequence'])
        #dataset_class = DatasetVisibilityKittiSingle
        dataset_class = DatasetVisibilityMidairSingle
    ...
    for epoch in range(starting_epoch, _config['epochs'] + 1):
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            for idx in range(len(sample['rgb'])):
                ...
                depth_img_no_occlusion = torch.zeros_like(depth_img, device='cuda', dtype=torch.float)
                #occlusion_threshold = torch.tensor(occlusion_threshold, dtype=torch.float32)
                #print('depth_img type: ', depth_img.dtype)
                #print('cam_params type: ', cam_params.dtype)
                #print('depth_img_no_occ type: ', depth_img_no_occlusion.dtype)
                #print('depth_img type: ', depth_img.shape[1].dtype, ' x ', depth_img.shape[0].dtype)
